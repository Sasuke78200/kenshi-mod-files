# Kenshi Game Data Constants

---

## Item Flags

The `flags` field (`u32`, present for `file_version >= 15`) packs two values:

```
bits [31:4]  save_counter        // incremented each time the item is saved in the Construction Set
bits  [3:0]  item_load_flags
```
...
|------------|-----|-------|-------------|
| `MODIFIED` | 0   | `1`   | Item has been modified relative to the base mod |
| `RENAMED`  | 1   | `2`   | Item has been renamed — always mirrors bit 0 |

> Bit 1 is not independent: it is always set when bit 0 is set, and always cleared when bit 0 is
> cleared.
...
In a reference triple `(v0, v1, v2)` (present for `file_version >= 10`), when all three values
equal `0x7FFFFFFF` (`INT32_MAX`), it marks a **deletion**: the reference should be removed from
the target game object rather than added.
